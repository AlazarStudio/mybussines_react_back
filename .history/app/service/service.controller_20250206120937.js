...
export const createNewService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(centerIds) || centerIds.length === 0) {
      console.error(
        '❌ Ошибка: centerIds должен быть непустым массивом, получено:',
        centerIds
      );
      return res.status(400).json({ error: 'centerIds must be a non-empty array' });
    }
    ...
    console.log('✅ Сервис создан:', service);

    // ✅ Добавление центров в промежуточную таблицу `ServiceOnCenters`
    ...
    console.log('✅ Связи с центрами добавлены');

    // 🔄 Получаем обновленный сервис с центрами
...
export const updateService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Удаляем старые связи в таблице `ServiceOnCenters`
      ...
      data: {
        ...
        centers: {
          create: centerIds.map((centerId) => ({
            center: { connect: { id: parseInt(centerId, 10) } },
          })),
        },
      },
      include: {
