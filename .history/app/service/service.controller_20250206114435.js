  ...
  try {
    const service = await prisma.service.create({
      data: {
        ...
        img: images,
        centers: {
          connect: centerIds.map((id) => ({ centerId: parseInt(id, 10) })), // ✅ Подключаем несколько центров
        },
...
export const updateService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Обновление связей центров
    await prisma.serviceOnCenters.deleteMany({
      where: { serviceId: parseInt(id, 10) }, // Удаляем старые связи
    });
