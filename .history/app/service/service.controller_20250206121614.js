...
export const getServices = asyncHandler(async (req, res) => {
  ...
  const services = await prisma.service.findMany({
    ...
    include: {
      centers: { include: { center: true } }, // ✅ Получаем связанные центры
      form: true,
    },
  });
...
export const createNewService = asyncHandler(async (req, res) => {
  ...
  try {
    const images = Array.isArray(img)
      ? img.filter((image) => image !== null).map((image) =>
...
export const updateService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await prisma.serviceOnCenters.deleteMany({ where: { serviceId: parseInt(id, 10) } });
    await prisma.serviceOnCenters.createMany({
      data: centerIds.map((centerId) => ({
