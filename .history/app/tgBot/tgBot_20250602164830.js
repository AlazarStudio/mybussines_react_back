...
bot.on('channel_post', async (msg) => {
  ...
  // сначала пробуем найти двойной или более перенос строки
  const splitMatch = rawText.match(/^([\s\S]*?)(\r?\n){2,}([\s\S]*)$/);

  let title;
  let description;

  if (splitMatch) {
    title = splitMatch[1].trim().slice(0, 100);
    description = splitMatch[3].trim();
  } else {
    // если переносов нет, просто делим по длине
    title = rawText.trim().slice(0, 100);
    description = rawText.trim().slice(100).trim();
  }
