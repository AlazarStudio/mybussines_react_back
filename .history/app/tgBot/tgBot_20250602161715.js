...
bot.on('channel_post', async (msg) => {
  ...
  // Корректное разделение на title и description
  let title = '';
  let description = '';
  const newlineIndex = rawText.indexOf('\n');

  if (newlineIndex === -1) {
    title = rawText.slice(0, 100).trim();
    description = '';
  } else {
    title = rawText.slice(0, newlineIndex).trim().slice(0, 100);
    description = rawText.slice(newlineIndex + 1).trim();
  }
