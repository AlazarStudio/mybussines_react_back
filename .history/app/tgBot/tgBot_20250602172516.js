...
bot.on('channel_post', async (msg) => {
  ...
  const fullText = [msg.text, msg.caption].filter(Boolean).join('\n').trim();

  // Проверка наличия хэштега
  if (!fullText.toLowerCase().includes('#на_сайт')) {
    console.log('⛔️ Пропущено: нет хэштега #на_сайт');
    return;
  }

  // Удаляем хэштег из текста
  const cleanedText = fullText.replace(/#на_сайт/gi, '').trim();
